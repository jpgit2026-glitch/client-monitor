...
const PRIORITY_STYLE: Record<string, string> = {
  URGENT: "bg-rust/10 text-rust", HIGH: "bg-amber/10 text-amber", NORMAL: "", LOW: "bg-ink/5 text-ink/35",
};
...
export default function TaskModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[5vh] pb-[5vh]" onClick={onClose}>
      ...
      <div
        className="relative bg-white rounded-2xl shadow-elevated w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-rule/50"
      ...
      >
        {loading ? (
          ...
          <>
            {/* View mode header */}
            <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-rule/40 px-7 py-5 rounded-t-2xl z-10">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-1.5 text-xs text-muted">
                    ...
                    {task.createdBy && <><span className="text-rule">&middot;</span><span>from {task.createdBy.name}</span></>}
                  </div>
                </div>
            ...
            <div className="px-7 py-6 space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {task.assignedTo && (
                  <span className="inline-flex items-center gap-1.5 text-xs text-muted">
                    <span className="w-5 h-5 rounded-full bg-green-100 text-green-700 flex items-center justify-center text-[10px] font-bold">{task.assignedTo.name.charAt(0)}</span>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-green-700">{task.progressPct}%</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {task.dueDate && (
                  <div className="bg-green-50/40 rounded-lg px-4 py-3">
              ...
              {blocked && (
                <div className="rounded-lg bg-amber/8 border border-amber/15 px-4 py-3">
              ...
              {/* Notes */}
              {task.notes && (
                <div className="bg-green-50/60 rounded-lg px-4 py-3 border border-green-100/60">
                  <div className="text-2xs text-green-600 font-semibold uppercase tracking-wider mb-1.5">Notes</div>
                  <p className="text-sm text-ink/70 leading-relaxed">{task.notes}</p>
                </div>
              )}
                ...
                <div className="space-y-4 mb-4">
                  {task.comments.map((c) => (
                    <div key={c.id} className="flex gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-green-100 text-green-700 flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-ink/70 mt-0.5 leading-relaxed">{c.message}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* View mode footer */}
            <div className="sticky bottom-0 bg-white/95 backdrop-blur-sm border-t border-rule/40 px-7 py-4 rounded-b-2xl flex justify-end gap-2">
